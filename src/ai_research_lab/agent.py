...
from .claim_extractor import load_or_create_claim
...
from .paths import ensure_within_root, portable_relative_path, project_root, study_paths
...
class ResearchAgent:
    ...
    def run(self, study_name: str) -> ExperimentResult:
        ...
        try:
            ...
            runner_artifact_paths = [Path(path) for path in result.artifact_paths]
            registry_artifact_paths = [claim_path, plan_path, *runner_artifact_paths]
            final_status = self._evaluate_result(result)
            artifacts = register_artifacts(study_root, plan.experiment_id, registry_artifact_paths)
            logbook_path = build_logbook(study_root, study_name, claim, plan, result, artifacts)
            register_artifacts(study_root, plan.experiment_id, registry_artifact_paths + [logbook_path])
            result = result.model_copy(
                update={
                    "status": final_status,
                    "artifact_paths": [
                        portable_relative_path(path, self.root_dir)
